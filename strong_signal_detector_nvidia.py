...
# data = yf.download("BABA", start="2017-01-01", end='2017-12-11')
# data.Close.plot()
data = pdr.get_data_yahoo("BABA", start="2017-01-01", end=end_date)
...
# grid_sell_thres = [30, 35, 40, 45, 50]
grid_buy_thres = [-40, -45]
lower_buy_thres = -45  # if percentage decrease is more than this, buy more <shares_to_buy> shares
upper_sell_thres = 45  # if percentage increase is more than this, sell <shares_to_sell> shares
shares_to_buy = 35  # number of shares to buy when buying opportunity
shares_to_sell = 35  # number of shares to sell when selling opportunity
best_money_earned = 0
...
for lower_buy_thres in grid_buy_thres:
    ...
    for idx, row in df.iterrows():
        ...
        # get index 3 weeks ago
        rolling_idx = max(idx - 1, df.index[0])
        ...
        elif rolling_percent_inc <= lower_buy_thres:
    ...
    print(
        "If you bought at original close price 143.80.. you would be able to get ${0}".format(
            orig_money / 143.80 * close))
