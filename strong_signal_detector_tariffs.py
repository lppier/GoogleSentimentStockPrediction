...
data = yf.download("BABA", start="2017-01-01", end="2018-12-6")
# data.Close.plot()
...
df = df.rename(columns={"tariffs": "tariffs"})
df['Week'] = df.index
df = df.reset_index()
...
shares_to_buy = 20  # number of shares to buy when buying opportunity
shares_to_sell = 20  # number of shares to sell when selling opportunity
best_money_earned = 0
...
for upper_sell_thres in grid_sell_thres:
    ...
    for idx, row in df.iterrows():
        ...
        rolling_trend = rolling_row['tariffs'].values[0]
    ...
    print(
        "If you bought at original close price 170.57.. you would be able to get ${0}".format(orig_money / 143 * close))
